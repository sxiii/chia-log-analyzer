...
# Go around all text files in directory
for filename in *txt; do
	...
	if [ -z "$dateend" ]; then (( x++ )); else printf "$filename \t $threads \t $ram \t $stage1 \t $stage2 \t $stage3 \t $stage4 \t $totaltime \t $copytime \t $datestart \t $dateend \n" >> result.tsv; fi
	
done

cat result.tsv

echo "Skipped $x files because plotting process was cancelled (wasn't finished completely)"
